...
**Soil sampling** during each of the 4 campaigns will make use of the same soil core used in the Berlese trap.  We will quantify 4 soil characteristics:

1) Soil moisture will be measured by weighing soil cores before placing them in the Berlese traps (which will partially dry them) and after fully drying them.
1) Soil bulk density will be used to evaluate porosity.
2) Soil texture will be measured using a hydrometer.
...
We request that a single rebar stake remain in place to mark the location of plots.
...
During each campaign plots will only be visited 2 to 3 times: once for set-up, once for take-down, and a potential third time if not all data (particularly plant surveys) can be collected within two days.
